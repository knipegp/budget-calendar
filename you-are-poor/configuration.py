...
class AppConfiguration:
    def __init__(self, app_name, app_dir, force_write=False):
        ...
        self.transaction_directory = os.path.join(
            self.app_directory, "transactions", "new"
        )
        self.saved_transaction_directory = os.path.join(
            self.app_directory, "transactions", "saved"
        )
    ...
    def create_config(self, force=False):
        ...
        for account in self.accounts:
            self.starting_bal[account] = float(
                utils.get_free_answer(
                    "Enter starting balance for account {}: ".format(account)
                )
            )

    def write_config(self):
        write_dict = {"accounts": self.accounts, "starting_bal": self.starting_bal}
    ...
    def add_account(self, account):
        ...
        # ans = utils.get_binary_answer(
        #    'Use default transaction descriptors for account {}?'.format(account)
        # )

        # if not ans:
        #     ans = utils.get_free_answer(
        #       'Enter account transaction descriptor: '.format(default_descriptors), 0
        # )
